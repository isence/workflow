...
module.exports = function (gulp, config) {
    ...
    var copyHandler = function (type, file) {
        ...
        return gulp.src(file, {
            base: paths.src.dir
        }).pipe(gulp.dest(paths.dev.dir)).on('end', reloadHandler);
    };
    // 自动刷新
    ...
    function compileLess() {
        //console.log('less');
        return gulp.src(paths.src.less).pipe(less({
            relativeUrls: true
        })).on('error', function (error) {
            console.log(error.message);
        }).pipe(lazyImageCSS({
            imagePath: lazyDir
        })).pipe(gulp.dest(paths.dev.css)).on('data', function () {}).on('end', reloadHandler)
    }
    //编译 sass
    function compileSass() {
        return gulp.src(paths.src.sass).pipe(sass()).on('error', sass.logError).pipe(lazyImageCSS({
            imagePath: lazyDir
        })).pipe(gulp.dest(paths.dev.css)).on('data', function () {}).on('end', reloadHandler)
    }
    //编译 html
    function compileHtml() {
        return gulp.src(paths.src.html).pipe(ejs(ejshelper()).on('error', function (error) {
            console.log(error.message);
        })).pipe(gulp.dest(paths.dev.html)).on('data', function () {}).on('end', reloadHandler)
    }
    //编译 JS
    function compileJs() {
        var condition = webpackConfig ? true : false;
        return gulp.src(paths.src.js).pipe(gulpif(condition, webpack(webpackConfig), babel({
            presets: ['es2015', 'stage-2']
        }))).pipe(gulp.dest(paths.dev.js)).on('end', reloadHandler)
    }
    //启动 livereload
    ...
    //注册 build_dev 任务
    gulp.task('build_dev', gulp.series(delDev, gulp.parallel(copyImg, copySlice, //compileJs,
        copyJs, copyFont, copyLib, copyMedia, compileLess, compileSass, compileHtml), gulp.parallel(watch, loadPlugin), startServer));
};
